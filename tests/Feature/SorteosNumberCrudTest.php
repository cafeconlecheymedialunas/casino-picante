...
class SorteosNumberCrudTest extends TestCase
{
    ...
    public function test_winning_numbers_are_saved_per_prize(): void
    {
        $line = Line::create([
            'name' => 'Linea Test',
            'status' => 'active',
        ...
        $raffle = Raffle::withoutGlobalScopes()->create([
            ...
            'line_id' => $line->id,
            'prizes' => [
                ['position' => 1, 'name' => 'Primer premio', 'image' => null],
                ['position' => 2, 'name' => 'Segundo premio', 'image' => null],
            ],
        ]);
        $raffle->lines()->sync([$line->id]);
        ...
        Livewire::test(Sorteos::class)
            ->call('openWinnerModal', $raffle->id)
            ->set('winnerPrizes.0.winner_number', '3')
            ->set('winnerPrizes.1.winner_number', '8')
            ->call('saveWinner')
            ->assertHasNoErrors();
        ...
        $this->assertSame(3, $prizes[0]['winner_number']);
        $this->assertSame($firstClient->id, $prizes[0]['winner_user_id']);
        $this->assertSame($line->id, $prizes[0]['winner_line_id']);
        $this->assertSame(2, $prizes[0]['winner_participations_count']);

        $this->assertSame(8, $prizes[1]['winner_number']);
        $this->assertSame($secondClient->id, $prizes[1]['winner_user_id']);
        $this->assertSame(1, $prizes[1]['winner_participations_count']);

        $this->assertSame($firstClient->id, $raffle->winner_user_id);
        $this->assertSame(3, $raffle->winner_number);
    }
