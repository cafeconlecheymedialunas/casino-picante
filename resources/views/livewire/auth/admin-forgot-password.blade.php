<div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;background:radial-gradient(ellipse at 50% 0%,rgba(255,106,26,.08) 0%,transparent 60%),var(--black);">
    <div style="width:min(440px,100%);">
        <div style="text-align:center;margin-bottom:32px;">
            ...
            <div style="color:var(--muted-2);font-size:11px;font-weight:700;letter-spacing:.15em;text-transform:uppercase;margin-top:6px;">Recuperar contraseña</div>
        </div>
        @if($sent)
        <div style="background:linear-gradient(180deg,#1c0e0e,#120909);border:1px solid var(--good);border-radius:14px;padding:32px 28px;text-align:center;">
            ...
            <p style="color:var(--muted-2);font-size:13px;line-height:1.5;margin-bottom:20px;">Enviamos un enlace a <strong style="color:var(--white)">{{ $email }}</strong>. Si no lo encontrás, revisá spam.</p>
            <a href="{{ route('admin.login') }}" wire:navigate class="btn-link">Volver al login</a>
        </div>
        @else
        <div style="background:linear-gradient(180deg,#1c0e0e,#120909);border:1px solid var(--line-2);border-radius:14px;padding:32px 28px;position:relative;overflow:hidden;">
            ...
            <p style="color:var(--muted-2);font-size:13px;margin-bottom:22px;line-height:1.5;">Ingresá tu email y te enviaremos un enlace para restablecer tu contraseña.</p>
